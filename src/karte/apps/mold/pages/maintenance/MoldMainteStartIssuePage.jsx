...
import { clearMoldId, mainteInputAdd } from 'karte/apps/mold/reducers/mold-maintenance-reducer';


export class MoldMainteStartIssuePage extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      dict: {
  ...
  onBackClick() {
    //金型メンテナンスサブメニューに戻る
    // this.$f7.views.main.router.navigate(APP_DIR_PATH + '/mold-mainte-start', {pushState:true});
    this.$f7router.back();
  }
  ...
  renderCards() {
    ...
    if(!this.props.issueList) {
      this.$f7.views.main.router.navigate(APP_DIR_PATH + '/mold-mainte-sub-menu', {reloadAll: true});
      return;
    }
...
function mapStateToProps(state) {
  return {
    ...
    issueList: state.mold.moldMaintenance.issueList
  };
}
